...
# Optional tools for verification: pdfminer for character positions
# Declare a permissive placeholder for LTChar so static type checkers accept
# assignments from the real pdfminer type when it is available.
LTChar: Any

try:
    from pdfminer.high_level import extract_pages
    from pdfminer.layout import LTChar as _PdfMinerLTChar
    # assign the concrete pdfminer type to the permissive name
    LTChar = _PdfMinerLTChar
    PDFMINER_AVAILABLE = True
except Exception:
    PDFMINER_AVAILABLE = False

    # Provide a fallback for `extract_pages` which raises a clear runtime
    # error if the optional dependency is missing.
    def extract_pages(*args, **kwargs) -> Iterator[Any]:
        raise RuntimeError("pdfminer.six is required for X extraction. Install it to use this feature.")

    # Keep LTChar as a permissive Any-typed name in the failure path.
    LTChar = Any
...
def validate_against_structure(data: Dict[str, Any], structure_path: Path) -> None:
    ...
    for section in structure:
        ...
        # Normalize to a list of dicts to allow uniform validation for both
        # single-object sections and array sections. If the section exists but
        # isn't a dict/list (primitive), skip field-level checks.
        items_to_check: List[Dict[str, Any]] = []
        if isinstance(section_data, dict):
            items_to_check = [section_data]
        elif isinstance(section_data, list):
            # Validate each item in the list (e.g. beneficiaries, goods)
            items_to_check = [item for item in section_data if isinstance(item, dict)]
        else:
            items_to_check = []
        ...
        for idx, item in enumerate(items_to_check):
            for field in section.get("fields", []):
                ...
                value = item.get(field_id) if isinstance(item, dict) else None
                if required_field and (value is None or value == ""):
                    if isinstance(section_data, list):
                        errors.append(f"Missing required field '{section_id}[{idx}].{field_id}'.")
                    else:
                        errors.append(f"Missing required field '{section_id}.{field_id}'.")
                    continue

                # Additional validation: allowed values
                allowed_values = field.get("values")
                if allowed_values is not None and value is not None and value != "":
                    if value not in allowed_values:
                        errors.append(
                            f"Invalid value for '{section_id}.{field_id}': '{value}' not in {allowed_values}"
                        )

                # Format-specific checks
                fmt = field.get("format") or field.get("format", None)
                if value is not None and value != "" and fmt:
                    try:
                        if fmt == "YYYY-MM-DD" or fmt == "date":
                            if not (isinstance(value, str) and len(value.split("-")) == 3):
                                raise ValueError()
                        if fmt == "decimal":
                            float(value)
                        if fmt == "integer":
                        ...
                        errors.append(f"Field '{section_id}.{field_id}' does not match format '{fmt}': '{value}'")

                # Dependency checks: if this field depends on another, ensure dependency holds
                depends_on = field.get("dependsOn")
                if depends_on and value not in (None, ""):
                    # depends_on is a key inside the same section
                    dep_val = item.get(depends_on) if isinstance(item, dict) else None
                    if not bool(dep_val):
                        errors.append(
                            f"Field '{section_id}.{field_id}' provided but depends on '{section_id}.{depends_on}' being truthy"
                        )
...
def build_overlay(
    ...
    checkbox_log: Optional[List[Dict[str, Any]]] = None,
) -> PdfReader:
    ...
    for page_index in range(num_pages):
        ...
        for mapping in pages_by_index.get(page_index, []):
            ...
            if mapping.field_type == "checkbox":
                ...
                # 1) If it's already a boolean, use it.
                if isinstance(value, bool):
                    checked = value
                else:
                    # 2) Handle specific derived/textual cases.
                    # - Succession can be provided as a textual value under
                    #   `deceased.succession` (e.g. 'testada' / 'intestada').
                    if mapping.key.endswith("isTestate"):
                        succ = flattened_data.get("deceased.succession")
                        if isinstance(succ, str):
                            succ_norm = succ.strip().lower()
                            checked = succ_norm in ("testada", "testate", "con testamento", "with will", "will")
                        else:
                            checked = bool(value)
                    elif mapping.key.endswith("isIntestate"):
                        succ = flattened_data.get("deceased.succession")
                        if isinstance(succ, str):
                            succ_norm = succ.strip().lower()
                            checked = succ_norm in ("intestada", "intestate", "sin testamento", "without will")
                        else:
                            checked = bool(value)
                    elif mapping.key.endswith("hasDisability"):
                        # hasDisability can be textual, boolean or inferred from
                        # `subject.disabilityPercentage` if the boolean is absent.
                        if isinstance(value, str):
                            checked = value.strip().lower() in ("si", "sí", "true", "yes", "1", "x")
                        elif value is None:
                            perc = flattened_data.get("subject.disabilityPercentage")
                            if perc is None:
                                checked = False
                            else:
                                try:
                                    checked = float(perc) > 0
                                except Exception:
                                    checked = False
                        else:
                            checked = bool(value)
                    else:
                        # Generic textual truthy checks for other checkboxes.
                        if isinstance(value, str):
                            checked = value.strip().lower() in ("si", "sí", "true", "yes", "1", "x")
                        else:
                            checked = bool(value)

                # Record the checkbox evaluation (one entry per page/mapping)
                if checkbox_log is not None:
                    checkbox_log.append(
                        {
                            "key": mapping.key,
                            "page_index": page_index,
                            "x": mapping.x,
                            "y_from_top": mapping.y_from_top,
            ...
            # Determine font name and attempt to auto-fit font size to available width
            font_name = mapping.font_name or "Helvetica"
            font_size = mapping.font_size

            # Compute available width: prefer mapping.max_width, otherwise use page width minus x minus margin
            margin_right = 36
            available_width = mapping.max_width if mapping.max_width is not None else max(36, width - mapping.x - margin_right)

            # Reduce font size until text fits or min font size reached
            min_font_size = 6
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional path to write checkbox log as JSONL (one JSON object per line).",
    )
    parser.add_argument(
        "--extract-x",
        action="store_true",
        help="Also extract character 'X' positions from the generated PDF (requires pdfminer.six).",
    )
    parser.add_argument(
        "--match-threshold",
        type=float,
        default=None,
        help="If provided, match FIELD_MAPPINGS to extracted X positions using this threshold (points).",
    )
    return parser.parse_args()


def extract_x_positions_from_pdf(pdf_path: Path, out_path: Path) -> int:
    if not PDFMINER_AVAILABLE:
        raise RuntimeError("pdfminer.six is required for X extraction. Install via requirements.txt")
    ...
    for page_index, page_layout in enumerate(extract_pages(str(pdf_path))):
        ...
        while stack:
            node = stack.pop()
            # pdfminer uses LTChar for characters
            if isinstance(node, LTChar):
                ch = node.get_text()
                if ch in ("X", "x"):
                    x0, y0, x1, y1 = node.bbox
                    y_from_top = page_height - y0
                    results.append(
                        {
                            "pdf": str(pdf_path),
                            "page_index": page_index,
                ...
                if hasattr(node, "_objs"):
                    stack.extend(node._objs)
    ...
    print(f"Wrote {len(results)} X positions to {out_path}")
    return len(results)
...
def match_mappings_to_x_positions(x_positions_path: Path, threshold: float = 5.0, mappings: Sequence[FieldMapping] = FIELD_MAPPINGS) -> Dict[str, Any]:
    x_positions = load_x_positions(x_positions_path)

    # index positions by page for faster lookup
    by_page = {}
    for item in x_positions:
        by_page.setdefault(item.get("page_index"), []).append(item)
    ...
    for mapping in mappings:
        for page in mapping.pages:
            total += 1
            expected_x = float(mapping.x)
            expected_y = float(mapping.y_from_top)
            candidates = by_page.get(page, [])
            best = None
            best_dist = float("inf")
            for c in candidates:
                # use x0 and y_from_top from extraction
                dx = (c.get("x0", 0) - expected_x)
                dy = (c.get("y_from_top", 0) - expected_y)
                dist = (dx * dx + dy * dy) ** 0.5
                if dist < best_dist:
                    best_dist = dist
                    best = c

            matched = best is not None and best_dist <= float(threshold)
            if matched:
                found += 1
                sum_dist += best_dist
            ...
            details.append(
                {
                    ...
                    "expected_y_from_top": expected_y,
                    "found": bool(matched),
                    "min_dist": best_dist if best is not None else None,
                    "matched_char": best,
                }
            )

    avg = (sum_dist / found) if found else 0.0
    report = {
        "file": str(x_positions_path),
        "threshold": float(threshold),
        "total_checkbox_mappings": total,
        "found_mappings": found,
        "not_found_mappings": total - found,
        "avg_distance_for_matches": avg,
        "details": details,
    }
    # write report next to x_positions file
    out_report = x_positions_path.with_name(x_positions_path.stem + "_mapping_report.json")
...
def main() -> None:
    ...
    checkbox_log: List[Dict[str, Any]] = []
    overlay_reader = build_overlay(flat, FIELD_MAPPINGS, page_sizes, checkbox_log=checkbox_log)
    ...
    # If requested, write checkbox evaluation log as JSON lines
    if args.log_checks:
        args.log_checks.parent.mkdir(parents=True, exist_ok=True)
        with args.log_checks.open("w", encoding="utf-8") as fh:
            for entry in checkbox_log:
                fh.write(json.dumps(entry, ensure_ascii=False) + "\n")
        print(f"Wrote checkbox log to {args.log_checks}")

    # Optional: extract X positions and/or match them to mappings
    if getattr(args, "extract_x", False):
        x_out = output_path.with_name(output_path.stem + "_x_positions.jsonl")
        try:
            extract_x_positions_from_pdf(output_path, x_out)
        except Exception as exc:
            print(f"X extraction failed: {exc}")

        if args.match_threshold is not None:
            try:
                match_mappings_to_x_positions(x_out, threshold=args.match_threshold)
            except Exception as exc:
